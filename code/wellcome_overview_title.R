...
overview<-final %>% select(archive_name, title, age_range, authors, year,treatment , duration, country, intervention_value, outcome_category, z_treatment_effect) %>%
  ...
  filter(!is.na(z_treatment_effect)) %>% select(-z_treatment_effect) %>% filter(treatment !='Savings')
final<- final %>% filter(treatment !='Savings')
...
# population 
part6<- final %>% select(archive_name, authors, year,title,population) %>%
  mutate_at('population', as.character) %>% mutate_at('population', capitalize) %>% unique()
...
# age_range
part8<- final %>% select(archive_name, authors, year,title,age_range) %>% unique() %>%
  mutate_at('age_range', as.character)
...
overview<-overview  %>%
  ...
  select(-c('archive_name','year')) %>%mutate(all_n=round(all_n, 0))%>% mutate_at('all_n', as.character)

overview<-overview[order(overview$authors),]

names(overview)<-c('Study',"Title","Intervention","Average transfer value (USD)","Outcomes", "Delay Intervention--Survey",
                   "Target \n population", "Age \n range", "Country", "Sample \n size")
...
overview.tex <- xtable(overview, 
                         align=c("p{0.1cm}",
                                 "p{1.3cm}", #columnn study
                                 "p{2.5cm}", #columnn title
                                 "p{1.4cm}", #ntervention
                                 "p{1.3 cm}", #cost
                                 "p{1.8cm}", #outcomes
                                 "p{1.7cm}", #delay
