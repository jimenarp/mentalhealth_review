...
clean <- clean %>% 
  ...
  mutate(treatment_n=ifelse(is.na(treatment_n), all_n/2,treatment_n)) %>%
  mutate(control_n=ifelse(is.na(control_n), all_n/2,control_n)) %>%
  ...
  mutate(z_se=ifelse(standarized==0,((treatment_n+control_n)/(treatment_n*control_n)+(z_treatment_effect)^2/(2*(treatment_n+control_n-2)))^.5,z_se))%>%
