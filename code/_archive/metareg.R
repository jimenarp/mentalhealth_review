...
metareg(test,  age + LMIC + lumpsum + female_share   + intervention_gdppercapita + duration_years+age_missing+female_missing+intervention_gdppercapita_missing+duration_years_missing)
metareg(test,  age + LMIC + lumpsum + female_share   + intervention_peryear_gddpercapita + duration_years+age_missing+female_missing+duration_years_missing+intervention_peryear_gddpercapita_missing+gdppercapita)
metareg(test,  age + + lumpsum + female_share   + intervention_peryear_gddpercapita + duration_years+age_missing+female_missing+duration_years_missing+intervention_peryear_gddpercapita_missing+gdppercapita)
...
a<-metareg(test,  age + LMIC + lumpsum + female_share   + intervention_peryear_gddpercapita + duration_years+age_missing+female_missing+duration_years_missing+intervention_peryear_gddpercapita_missing)
...
a<-rma(yi = z_treatment_effect, 
    sei = z_se, 
    data = reg, 
    method = "ML", 
    mods = ~ age + LMIC + lumpsum + female_share   + intervention_peryear_gddpercapita + duration_years+age_missing+female_missing+duration_years_missing+intervention_peryear_gddpercapita_missing+gdppercapita, 
    test = "z")

robust(a, cluster=reg$id)
test<-metagen(z_treatment_effect,
              z_se,
              data = final,
              studlab = archive_name,
...
a<-final %>% filter(int_Neighborhood==1) %>% filter(lumpsum==1)
...
temp_coef_female_grad<-toformat(temp_reg$beta['female_share',1], decimals)
temp_coef_cost_grad<-toformat(temp_reg$beta['intervention_gdppercapita',1], decimals)
...
temp_se_female_grad<-toformat(temp_reg$se[4], decimals)
temp_se_cost_grad<-toformat(temp_reg$se[5], decimals)
temp_se_delay_grad<-toformat(temp_reg$se[6], decimals)

temp_p_intrcpt_grad<-temp_reg$pval[1]
temp_p_age_grad<-temp_reg$pval[2]
temp_p_lmic_grad<-'-'
temp_p_lumpsum_grad<-temp_reg$pval[3]
temp_p_female_grad<-temp_reg$pval[4]
temp_p_cost_grad<-temp_reg$pval[5]
temp_p_delay_grad<-temp_reg$pval[6]
...
temp_se_female_grad<- paste("(",temp_se_female_grad,")",sep="")
temp_se_female_grad<-p.stars(temp_p_female_grad, temp_se_female_grad)

temp_se_cost_grad<- paste("(",temp_se_cost_grad,")",sep="")
temp_se_cost_grad<-p.stars(temp_p_cost_grad, temp_se_cost_grad)
...
temp_coef_female_lottery<-toformat(temp_reg$beta['female_share',1], decimals)
temp_coef_cost_lottery<-toformat(temp_reg$beta['intervention_gdppercapita',1], decimals)
temp_coef_delay_lottery<-toformat(temp_reg$beta['duration_years',1], decimals)
...
temp_se_female_lottery<-toformat(temp_reg$se[3], decimals)
temp_se_cost_lottery<-toformat(temp_reg$se[4], decimals)
temp_se_delay_lottery<-toformat(temp_reg$se[5], decimals)

temp_p_intrcpt_lottery<-temp_reg$pval[1]
temp_p_age_lottery<-temp_reg$pval[2]
temp_p_lmic_lottery<-'-'
temp_p_lumpsum_lottery<-'-'
temp_p_female_lottery<-temp_reg$pval[3]
temp_p_cost_lottery<-temp_reg$pval[4]
temp_p_delay_lottery<-temp_reg$pval[5]
...
temp_se_female_lottery<- paste("(",temp_se_female_lottery,")",sep="")
temp_se_female_lottery<-p.stars(temp_p_female_lottery, temp_se_female_lottery)

temp_se_cost_lottery<- paste("(",temp_se_cost_lottery,")",sep="")
temp_se_cost_lottery<-p.stars(temp_p_cost_lottery, temp_se_cost_lottery)

temp_se_delay_lottery<- paste("(",temp_se_delay_lottery,")",sep="")
temp_se_delay_lottery<-p.stars(temp_p_delay_lottery, temp_se_delay_lottery)
...
temp_coef_lumpsum_asset<-toformat(temp_reg$beta['lumpsum',1], decimals)
...
temp_se_lmic_asset<-'-'
temp_se_lumpsum_asset<-toformat(temp_reg$se[3], decimals)
temp_se_female_asset<-toformat(temp_reg$se[4], decimals)
temp_se_cost_asset<-'-'
temp_se_delay_asset<-toformat(temp_reg$se[5], decimals)

temp_p_intrcpt_asset<-temp_reg$pval[1]
temp_p_age_asset<-temp_reg$pval[2]
temp_p_lmic_asset<-'-'
temp_p_lumpsum_asset<-temp_reg$pval[3]
temp_p_female_asset<-temp_reg$pval[4]
temp_p_cost_asset<-'-'
temp_p_delay_asset<-temp_reg$pval[5]
...
temp_se_lumpsum_asset<- paste("(",temp_se_lumpsum_asset,")",sep="")
temp_se_lumpsum_asset<-p.stars(temp_p_lumpsum_asset, temp_se_lumpsum_asset)
...
temp_coef_age_healthcare<- toformat(temp_reg$beta['age',1], decimals)
...
temp_se_age_healthcare<-toformat(temp_reg$se[2], decimals)
temp_se_lmic_healthcare<-'-'
temp_se_lumpsum_healthcare<-'-'
temp_se_female_healthcare<-toformat(temp_reg$se[3], decimals)
temp_se_cost_healthcare<-'-'
temp_se_delay_healthcare<-'-'

temp_p_intrcpt_healthcare<-temp_reg$pval[1]
temp_p_age_healthcare<-temp_reg$pval[2]
temp_p_lmic_healthcare<-
temp_p_lumpsum_healthcare<-'-'
temp_p_female_healthcare<-temp_reg$pval[3]
...
print(metareg.tex, file = "metareg.tex", compress = FALSE, 
