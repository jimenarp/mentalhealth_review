...
b<-clean %>% filter(authors=='Leventhal & Dupere') %>% select(population, control_n,outcome,outcome_dummy, treatment_effect, se_treatment_effect,end_sd_con,
                                                              end_mean_con,bas_sd_con,bas_mean_con, mean_input, se_input, z_treatment_effect, z_se,t_effect)
...
complete<-raw %>% 
  ...
  mutate(int=case_when(treatment_type=='UCT' ~ 'UCT', 
                       ...
                       treatment_type=='Savings'~'Savings',
                       treatment_type=='Neighborhood'~'Neighborhood',
                       treatment_type=='Healthcare'~'Healthcare')) %>% filter(outcome_category!='None')
