...
* Version 2: Under high levels of missingness, it can happen that a pair of two variables does not have
* any observations in common, e.g. because no person in a large survey answered two particular questions
* and thus no correlation between them can be calculated. Version 1 used to crash in that case.
* Version 2 spits out a warning and then calculated a non-weighted (arithmetic) mean instead of a weighted mean.

program define _gweightave2, rclass
	gettoken type 0 : 0
	gettoken h    0 : 0 
	gettoken eqs  0 : 0
	...
	* Number of variables
	...
	capture quietly correlate `varlist'

	if `r(N)' == 0 {
		noisily dis "Due to missing overlap, the `i' variables could not be weighted and the program proceded with a weight matrix of ones."
		mat Sigma = I(`i')
		global identity_matrix = 1
		}
	
	else {
		mat Sigma = r( C )
		global identity_matrix = 0
		}
