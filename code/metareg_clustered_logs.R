...
for(i in c('int_All', 'int_UCT','int_CCT', 'int_Neighborhood', 'int_Grad', 'int_Lottery', 'int_Asset', 'int_Healthcare')) {
    tryCatch({
      ...
      if (i=='int_Healthcare'){
        temp_reg<-rma(yi=z_treatment_effect, sei=z_se, slab=archive_name, data=temp_data, method='SJ',
                      mods = ~  age+ LMIC  +    + intervention_gdppercapita + duration_years)
      }
  ...
  consolidated<- data.frame("variable"=c("","Constant","","","Age","","","Low-/Middle-Income Country","","","Female Share","","","Lumpsum","","","Intervention Value","(as % of GDP per capita ","in logs)","Delay Intervention-Survey","(Years)","","Observations/Studies","" ),
...
print(metareg.tex, file = "tables/metareg_clustered_logs.tex", compress = FALSE, 
